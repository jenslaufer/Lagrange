...
The Data which was obtained in Step 1 is the oil, but right now it's raw oil. You need to clean, process and combine it to the data you need for analysis and modelling.
  ...
  - A Machine Learning Engineer combines data from different data sources, cleans and preprocesses data for a predictive model in a Juypter Notebook 
...
In the exploration phase, all you have to do is to understand what patterns and values are in the hands of the data. You want to make the results available to everyone interested.
  ...
  - Data Analysts build interactive web applications for high-level data exploring for interested stakeholders in , Dash, Tableau or Kibana. This way managers can find patterns on their own (danger zone!).
...
To train a neural networks you need a lot of GPU power. You need [Nvidia Docker](https://github.com/NVIDIA/nvidia-docker) for isolating the training process to a Docker container, as using GPU cannot be done in a hardware-agnostic and platform-agnostic way.
...
[https://vuejs.org/v2/cookbook/dockerize-vuejs-app.html](https://vuejs.org/v2/cookbook/dockerize-vuejs-app.html)
