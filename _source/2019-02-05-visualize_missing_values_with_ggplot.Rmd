...
```{r}
tt <- ttheme_default()
theme_set(theme_get() + theme(plot.title = element_text(size=21), axis.title.x = element_text(size=19), legend.text = element_text(size=17),
                              axis.title.y = element_text(size=19), axis.text.x  = element_text(size=17), axis.text.y = element_text(size=17)))
```
...
```{r echo=T, message=T, out.width='200%'}
df %>% 
  
  head(10) %>% 
  
  select(price, reviews_count, reviews, weight, sales, dim_x, dim_y, dim_z) %>%
...
```{r echo=T, message=T, out.width="100%"}
missing.values <- df %>%
...
missing.values %>% grid.table(theme=tt)
```
