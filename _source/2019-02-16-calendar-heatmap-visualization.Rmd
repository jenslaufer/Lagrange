...
heatmap <- function(df, var, title_label, fill_label, gradient_colours = c("darkblue", "grey", "darkred")) {
    df %>% 
      ...
      labs(x = "Day Of Week", y = "", fill = fill_label, title = title_label) + 
...
```{r}
heatmap.data <- read_csv("../_data/heatmap_data.csv")
```

```{r}
heatmap.data <- prepare.time.series(heatmap.data, 'date') %>% mutate(occupancy=occupancy*100)
```


```{r , fig.width=20, fig.height=8}
heatmap(heatmap.data, heatmap.data$occupancy, fill_label = 'Occupancy', title_label='Demo Heatmap')
```
