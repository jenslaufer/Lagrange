...
subtitle: "Training a shallow neural network on top of an InceptionV3 model on CIFAR-10 within Docker on an AWS GPU-instance"
...
- Persistence of models metadata on MongoDB for model versioning and reproducibility. 
...
You want to train the neural network on AWS. First, you need to set up an instance there. You can do this from the [AWS Web Console](https://console.aws.amazon.com/ec2/v2/home?region=us-east-1#Home:) or from the command line with the [AWS Command Line Client](https://github.com/aws/aws-cli).
...
You need a VPC-ID for the setup. You can use AWS command to get it:
...
You want to train the neural network with different training parameters to find the best set up. After the training, you want to test the model quality on the test set. It's a classification problem; I suggest to use the accuracy metric for simplicity. In the end, you want to persist the training log, model weights and architecture for further usage. Everything is reproducible and traceable this way. 
...
You open the mongo client. Then you are selecting the DB 'trainings' with the use command. You can add then training session with only 5% of the images, a rmsprop Optimizer with a
...
You can also query the database for the model files for a specific training session. You can see that we have hdf5 files for the model architecture and weights. There is also a JSON file with the training history you can use to analyse the training by itself. It can be used to visualise the training process. 
...
- A goal could be to remove the Python script from the container and load it as a python module from a repository in the entry point script. The entry point script uses environment variables, which are defined in the docker-compose file.
